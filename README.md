...
This project implements **Sense Matrices** — a framework that separates truth (`M_i`) from applicability/sense (`S_i`) within logical contexts (`W_i`). It enables semantic search, ambiguity detection, tautology detection, context composition, and logical validation of propositions.
...
6. **Matrix Layer** → `M_i` (truth) and `S_i` (sense/applicability)
7. **Operational Layer** → search, inference, disambiguation, composition

## Core concepts

- **M_i** — boolean matrix of truth values per object/property within context `W_i`
