"""Unified matrix engine for multi-context TKM reasoning."""
...
from unified_engine_core import Bridge, Context, SymbolRegistry, TruthValue
...
class UnifiedMatrixEngine:
    ...
    def __init__(
        ...
        self.Di: dict[
            str, jnp.ndarray
        ] = {}  # Discriminative Mask (Reducción Descriptiva)
    ...
    def get_context_mirror(self, context_name: str) -> str:
        ...
        for obj_id in ctx.objects:
            ...
            mirror.append(
                f" - Symbol: {obj_id} (Signs: {', '.join(signs)}) | Meta: {meta}"
            )
        ...
        for prop_id in ctx.properties:
            ...
            mirror.append(
                f" - Symbol: {prop_id} (Signs: {', '.join(signs)}) | Meta: {meta}"
            )

        return "\n".join(mirror)

    def add_dimension(
        self, context_name: str, symbol_id: str, sign: str = None, meta: dict = None
    ):
    ...
    def set_fact(
        ...
        if mode == "G":
            ...
            if status == "sinnlos":
                print(
                    f"[TKM-G] Warning: Fact ({subject_id}, {property_id}) is SINNLOS (Tautology/Contradiction). Integration skipped."
                )
                return
            if status == "unsinnig":
                print(
                    f"[TKM-G] Warning: Fact ({subject_id}, {property_id}) is UNSINNIG (Sense Violation). Integration rejected."
                )
                return
            ...
            for j, prop_name in enumerate(ctx.properties):
                ...
                if value is not None:
                    if isinstance(value, bool):
                        data = data.at[i, j].set(
                            self.M_T.value if value else self.M_F.value
                        )
            ...
            for j, prop_name in enumerate(ctx.properties):
                ...
                if requires:
                    ...
                    if req_context and req_context in self.contexts:
                        # Cross-context check
                        other_status = self.get_status(
                            obj_name, req_prop, context=req_context
                        )
                        if other_status.get("truth_label") != (
                            "TRUE" if req_value else "FALSE"
                        ):
                            S = S.at[i, j].set(False)
                        ...
                        if Vi[i, req_j] != (
                            self.M_T.value if req_value else self.M_F.value
                        ):
                            S = S.at[i, j].set(False)
        return S
    ...
    def _bool_mult(self, A: jnp.ndarray, B: jnp.ndarray) -> jnp.ndarray:
        return jnp.logical_or.reduce(
            jnp.logical_and(A[:, :, None], B[None, :, :]), axis=1
        )
    ...
    def get_omnirepresentation(self) -> jnp.ndarray:
        ...
        for ctx_name in self.contexts:
            ...
            omni = omni.at[
                current_idx : current_idx + size, current_idx : current_idx + size
            ].set(W_local)
            current_idx += size
        ...
        for bridge in self.bridges:
            ...
            if R is not None:
                if_indices = [
                    i
                    for i, x in enumerate(all_objs)
                    if x.startswith(f"{bridge.from_context}:")
                ]
                it_indices = [
                    i
                    for i, x in enumerate(all_objs)
                    if x.startswith(f"{bridge.to_context}:")
                ]
    ...
    def recursive_bridge_routing(
        self, start_context: str, steps: int = 3
    ) -> jnp.ndarray:
        """
    ...
    def get_collapsed_inference_plane(self) -> dict:
        ...
        return {"matrix": W_star.tolist(), "labels": all_objects}

    @staticmethod
    def load(path: str) -> UnifiedMatrixEngine:
        with open(path) as f:
            return UnifiedMatrixEngine.load_from_dict(yaml.safe_load(f))
    ...
    def load_from_dict(data: dict) -> UnifiedMatrixEngine:
        ...
        for cn, cd in data.get("contexts", {}).items():
            ...
            for obj_id in objs:
                registry.register_symbol(obj_id, obj_id)
            for prop_id in props:
                registry.register_symbol(prop_id, prop_id)
        ...
        bridges = [
            Bridge(
                name=bd.get("name", "bridge"),
                from_context=bd.get("from", ""),
                to_context=bd.get("to", ""),
                from_objects=bd.get("from_objects", []),
                to_objects=bd.get("to_objects", []),
                relation=bd.get("relation", "has_relation"),
            )
...
class TKMVisualizer:
    ...
    def export_context_matrix(self, context_name: str, file_path: str):
        ...
        if context_name not in self.engine.contexts:
            return
        self._ensure_parent_directory(file_path)
                ...
                "components": [
                    {
                        ...
                        "stages": {
                            "full_matrix": [
                                ...
                                if bool(Si[i, j])
                                and bool(Vi[i, j] == self.engine.M_T.value)
                            ]
                        },
                    }
    ...
    def _ensure_parent_directory(self, file_path: str) -> None:
        """Creates the parent directory for an export path when needed."""
